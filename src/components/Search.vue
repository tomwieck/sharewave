...
export default {
  ...
  methods: {
    ...
    clientCredentials: function() {
      var vm = this;
      vm.axios.get('https://sharewave.herokuapp.com/clientCredential')
      ...
      function () {
        ...
        if (/\S/.test(this.searchTerm)) {
          ...
          if (length !== 0) {
            ...
            var vm = this;
            vm.axios.get('https://sharewave.herokuapp.com/search?searchTerm=' + this.searchTerm + services + accessTokenString)
            .then(function (response) {
